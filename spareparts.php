...
<section id="spparts" class="section sppartsection content">
	<div class="container">
		...
		<form class="example" action="includes/search.inc.php" method="post">
			...
			<button type="submit" name="submit-search-usp"><img style="width:16px;height: 16px;" src="images/search-solid.svg"></button>
		</form>
				...
				<div class="col-md-10 bike-results">
					<div class="row">
						...
						if (isset($_GET['s'])) 
						{
							...
							$query = $_GET['s'];
							$searchsql = "SELECT * FROM post_ad WHERE ad_type = 'sparepart' AND (ad_title LIKE '%$query%' OR ad_description LIKE '%$query%' OR ad_price LIKE '%$query%' OR bike_make LIKE '%$query%') ORDER BY `ad_date` DESC LIMIT {$offset}, {$no_of_records_per_page};";
